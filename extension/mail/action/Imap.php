...
class Imap {
  ...
  public function changeFolder($folderName) {
    if ($this->ssl) {
      $address = '{' . $this->host . ':' . $this->port . '/imap/ssl/novalidate-cert}' . $folderName;
    } else {
  ...
  public function changeLoginInfo($host, $user, $pass, $port, $ssl, $folder) {
    if ($ssl) {
      $baseAddress = '{' . $host . ':' . $port . '/imap/ssl/novalidate-cert}';
    ...
    if ($mailbox = imap_open($address, $user, $pass, NULL, 0, array('DISABLE_AUTHENTICATOR' => 'CRAM-MD5') )) {
  ...
  public function search($type){
    $msg =  imap_search($this->mailbox, $type);
     // Fetch overview of mailbox.
     $fin = [];
     foreach($msg as $email){
         
          $overview = imap_fetch_overview($this->mailbox, $email);
          $overview = $overview[0];
          $o['m_id'] = $email;
          $o['subject'] = $overview->subject;
          array_push($fin, $o);
      }
  ...
  public function latest($date){
   $msg = imap_search($this->mailbox,"UID 1500 : *"  );
   $fin = [];
     foreach($msg as $email){
